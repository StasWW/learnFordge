...
/** Request body for `POST /api/ApiSchedule/{schoolPublicId}/events` (Teacher/Owner). */
export interface CreateScheduleEventDto {
  title: string;
  description: string | null;
  startUtc: string;
  endUtc: string;
  /** Optional client-suggested room; backend may allocate/override. */
  room?: string;
  attendeeUserPublicIds: string[];
}

/** Maps a wire DTO to the domain model consumed by the hooks/UI layer. */
